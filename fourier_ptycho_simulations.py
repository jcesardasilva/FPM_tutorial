...
def recovery_fpm(imSeqLowRes,arraysize,nloop,waveLength,spsize,psize,NA,LEDgap,LEDheight,epsilon = 0.1):
    ...
    for ii in range(nloop):
        ...
        for jj in range(arraysize**2):
            ...
            converIndex[ii] += np.abs(im_lowRes).mean()/(np.abs(im_lowRes-imSeqLowRes[ll]).sum())
            print(converIndex)
            im_lowRes = (m/m1**2)*imSeqLowRes[ll]*np.exp(1j*np.angle(im_lowRes))
...
if __name__=="__main__":
    ...
    arraysize = 15 # size of the LED array
    ...
    objectAmplitude = np.double(camera())
    #objectAmplitude = imresize(objectAmplitude,(256,256))
    objectAmplitude = resize(objectAmplitude,(256,256), mode='reflect')#/objectAmplitude.max()
    ...
    phase = (0.95*np.pi)*resize(phase,(256,256), mode='reflect')/phase.max()
    ...
    if a == '' or a == 'y':
        ...
        if a == '' or a == 'y':
            HigResImg = recovery_fpm(imSeqLowRes,arraysize,nloop,waveLength,spsize,psize,NA,LEDgap,LEDheight,epsilon)
        else:
